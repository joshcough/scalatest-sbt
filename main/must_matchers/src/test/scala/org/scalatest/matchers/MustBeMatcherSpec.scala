...
class MustBeMatcherSpec extends Spec with MustMatchers with Checkers with ReturnsNormallyThrowsAssertion with BookPropertyMatchers {
  ...
  describe("The BeMatcher syntax") {
    ...
    it("must do nothing if a BeMatcher matches") {
      1 must be (odd)
      2 must be (even)
    }

    it("must throw TestFailedException if a BeMatcher does not match") {
    ...
    it("must do nothing if a BeMatcher does not match, when used with not") {
      2 must not be (odd)
      1 must not be (even)
      22 must not (not (be (even)))
      1 must not (not (be (odd)))
    }

    it("must throw TestFailedException if a BeMatcher matches, when used with not") {
      ...
      val caught3 = intercept[TestFailedException] {
        6 must not (not (be (odd)))
      }
      assert(caught3.getMessage === "6 was even")
    }

    it("must do nothing if a BeMatcher matches, when used in a logical-and expression") {
      1 must (be (odd) and be (odd))
      1 must (be (odd) and (be (odd)))
      2 must (be (even) and be (even))
      2 must (be (even) and (be (even)))
    }

    it("must throw TestFailedException if at least one BeMatcher does not match, when used in a logical-or expression") {

      // both false
      val caught1 = intercept[TestFailedException] {
        2 must (be (odd) and be (odd))
      }
...
// TODO: Remember to try a BeMatcher[Any] one, to make sure it works on an Int
    ...
    it("must do nothing if at least one BeMatcher matches, when used in a logical-or expression") {

      // both true
      1 must (be (odd) or be (odd))
      1 must (be (odd) or (be (odd)))
      2 must (be (even) or be (even))
      2 must (be (even) or (be (even)))

      // first false
      1 must (be (even) or be (odd))
      1 must (be (even) or (be (odd)))
      2 must (be (odd) or be (even))
      2 must (be (odd) or (be (even)))

      // second false
      1 must (be (odd) or be (even))
      1 must (be (odd) or (be (even)))
      2 must (be (even) or be (odd))
      2 must (be (even) or (be (odd)))
    }

    it("must throw TestFailedException if a BeMatcher does not match, when used in a logical-or expression") {

      val caught1 = intercept[TestFailedException] {
        2 must (be (odd) or be (odd))
      }
    ...
    it("must do nothing if a BeMatcher does not match, when used in a logical-and expression with not") {
      2 must (not be (odd) and not be (odd))
      2 must (not be (odd) and not (be (odd)))
      2 must (not be (odd) and (not (be (odd))))
      1 must (not be (even) and not be (even))
      1 must (not be (even) and not (be (even)))
      1 must (not be (even) and (not (be (even))))
    }

    it("must throw TestFailedException if at least one BeMatcher matches, when used in a logical-and expression with not") {

      // both true
      val caught1 = intercept[TestFailedException] {
        1 must (not be (odd) and not be (odd))
      }
      ...
      val caught4 = intercept[TestFailedException] {
        2 must (not be (even) and not be (even))
      }
      ...
      // first false
      val caught7 = intercept[TestFailedException] {
        1 must (not be (even) and not be (odd))
      }
      ...
      val caught10 = intercept[TestFailedException] {
        2 must (not be (odd) and not be (even))
      }
      ...
      val caught13 = intercept[TestFailedException] {
        1 must (not be (odd) and not be (even))
      }
      ...
      val caught16 = intercept[TestFailedException] {
        2 must (not be (even) and not be (odd))
      }
    ...
    it("must do nothing if at least one BeMatcher doesn't match, when used in a logical-or expression when used with not") {

      // both false
      2 must (not be (odd) or not be (odd))
      2 must (not be (odd) or not (be (odd)))
      2 must (not be (odd) or (not (be (odd))))
      1 must (not be (even) or not be (even))
      1 must (not be (even) or not (be (even)))
      1 must (not be (even) or (not (be (even))))

      // first false
      1 must (not be (even) or not be (odd))
      1 must (not be (even) or not (be (odd)))
      1 must (not be (even) or (not (be (odd))))
      2 must (not be (odd) or not be (even))
      2 must (not be (odd) or not (be (even)))
      2 must (not be (odd) or (not (be (even))))

      // second false
      1 must (not be (odd) or not be (even))
      1 must (not be (odd) or not (be (even)))
      1 must (not be (odd) or (not (be (even))))
      2 must (not be (even) or not be (odd))
      2 must (not be (even) or not (be (odd)))
      2 must (not be (even) or (not (be (odd))))
    }

    it("must throw TestFailedException if both BeMatcher match, when used in a logical-or expression with not") {

      val caught1 = intercept[TestFailedException] {
        1 must (not be (odd) or not be (odd))
      }
      ...
      val caught4 = intercept[TestFailedException] {
        2 must (not be (even) or not be (even))
      }
    ...
    it("must work when the types aren't exactly the same") {
